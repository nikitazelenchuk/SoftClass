...
str.data.frame <- function(object, ...)
{
  ...
  cat("Размерность набора данных [объектов х переменных]: [", nrow(object), " x ",
      (p <- length(object)), "]", if(p != 1)"", if(p > 0)"",
      "\n", sep = "")
    ...
    if (is.null(object))
      ...
      if(is.vector(object)
        ...
        if(is.atomic(object)) {
          ...
          if     (mod == "nume")
            mod <- if(is.integer(object)) "Числовой"
          #else if(has.class) cl[1L] else "Числовой с плавающей точкой"
          else if(has.class) cl[1L] else "Числовой"
          else if(mod == "char") { mod <- "Символьный"; char.like <- TRUE }
          else if(mod == "comp") mod <- "Числовой (комплексные числа)" #- else: keep 'logi'
          if(is.array(object)) {
        ...
        lsep <- if(ord) "<" else ","
        str1 <-
          paste0(if(ord)" Ord.f" else " К",
                 "атегориальный w/ ", nl, " уровня", if(nl != 1) " (-ей)",
                 if(nl) " ",
